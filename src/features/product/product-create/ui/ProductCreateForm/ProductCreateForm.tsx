...
import { useCreateProduct, useProductCreateForm } from "../../model";

interface ProductCreateFormProps {
  onClose?: () => void;
}

export const ProductCreateForm: FC<ProductCreateFormProps> = ({ onClose }) => {
  const { createProduct, isLoading } = useCreateProduct();

  const { control, handleSubmit, formState, setValue } = useProductCreateForm();
  ...
  useEffect(() => {
    if (unitStorages === ProductUnitStoragesEnum.PIECES)
      setValue("unitPack", 1);
  }, [unitStorages]);
